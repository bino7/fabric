...
func (t *shop) Init(stub shim.ChaincodeStubInterface) pb.Response {
	return shim.Success([] byte(" success invok and Not opter !!!!!!!! "))
}

func (t *shop) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if opttype == "putvalue" { //设置
		...
		for keysIter.HasNext() {
			...
			tm := time.Unix(txtimesamp.Seconds, int64(txtimesamp.Nanos))
